...
extension GalleryImage: HTMLParsable {
    ...
    init?(element: Element) {
        ...
        guard let title = try? element.attr("data-title") else { return nil }
        self.title = title
        
        guard let date = try? element.attr("data-date").toDate() else { return nil }
        self.date = date
        
        guard let href = try? element.select("a").first()?.attr("href"),
            let pageURL = URL(string: href) else {
                return nil
        }
        self.pageURL = pageURL
        
        guard let src = try? element.select("img").first()?.attr("data-src"),
